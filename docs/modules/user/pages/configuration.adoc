...
Wolf can automatically spread apps across multiple GPUs instead of pinning everything to a single one.
Every render node found under `/dev/dri` (`renderD*`) is added to a pool, along with the `WOLF_RENDER_NODE` default, so single-GPU setups keep working unchanged.

When a new app (or lobby) starts, Wolf picks a GPU from the pool and remembers how many containers are currently using each one.
A GPU that has just been freed by a quitting app will therefore be preferred over one that is still busy — it is not a blind round-robin.

Wolf scores each candidate by how loaded that GPU *would be* after accepting the container, relative to its weight (internally `(usage + 1) / weight`), and picks the lowest score.
Scoring the resulting load rather than the current one is what makes a weight meaningful on an otherwise idle pool: it means a stronger GPU absorbs several apps in a row instead of only being preferred later on.

You can tune this behaviour in the `config.toml` file:

* `[gpus]` — an optional table mapping a render node path to an integer *weight* (a relative performance metric you supply).
A higher weight means more apps are balanced onto that GPU before Wolf spills over to the others, and while the GPUs are idle the highest-weighted one is picked first, regardless of the render node number.
For example, with an RTX 3090 and a 1660, weighting them 4:1 keeps four apps on the 3090 for every one on the 1660 (the first three go to the 3090 immediately):

[source,toml]
----
[gpus]
"/dev/dri/renderD128" = 4 # e.g. RTX 3090
"/dev/dri/renderD136" = 1 # e.g. GTX 1660
----

* `excluded_gpus` — an optional list of render node paths to remove from the pool entirely (for example an iGPU that drives your physical display).

[source,toml]
----
excluded_gpus = ["/dev/dri/renderD136"]
----

GPU assignment is decided by Wolf when a session starts and is fixed for the lifetime of that session; there is no per-app pinning.

Lobbies assign a single GPU once at creation (driven by the creating session) and all joining clients share it; the GPU is released when the lobby stops.

Because a lobby is shared, Wolf has to be careful about the *kind* of frames its desktop produces.
The zero-copy buffer formats (`CUDAMemory`, `DMABuf`) are device-local: a buffer belongs to one GPU and an encoder scoped to another GPU cannot read it, which shows up as a black screen with no obvious error.
Wolf pins the lobby to the creating session's GPU so that creator keeps its zero-copy path, but a *joining* client's GPU is chosen independently, so the shared desktop cannot emit device-local memory on a multi-GPU host.
Wolf therefore falls back to GPU-agnostic system memory (`video/x-raw`) for the lobby's desktop whenever the pool can host sessions on more than one GPU, and each client's encoder uploads to its own device.
On a single-GPU host the lobby keeps the configured zero-copy pipeline.

[TIP]
====
On NVIDIA hosts without a custom driver volume, Wolf scopes each container to only its assigned GPU (via `NVIDIA_VISIBLE_DEVICES` / `DeviceIDs`) so that multiple containers can run on different GPUs at the same time.
====
