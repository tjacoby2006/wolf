...
This means a GPU that has just been freed by a quitting app is reused before any other GPU is considered — it is not a blind round-robin.
...
A higher weight means more apps are balanced onto that GPU before Wolf spills over to the others.
For example, with an RTX 3090 and a 1660 you can send two or more apps to the 3090 first:
...
* Per-app pinning — any app can be pinned to a specific GPU with the `gpu_pin` field (a render node path).
A pinned app always uses that GPU and is not load-balanced. If the pinned GPU is excluded or missing, the app will not start.

[source,toml]
----
[[profiles.apps]]
title = "Steam"
gpu_pin = "/dev/dri/renderD128"
# ...
----
