...
|WOLF_RENDER_NODE
|/dev/dri/renderD128
|Fallback render node used when no GPU can be configured/detected; see: <<_multiple_gpu>>
...
<7> *gpus*: the list of GPUs Wolf may use, each with a `render_node` and a relative `weight`; if empty Wolf auto-detects all GPUs, see xref:_multiple_gpu[]
<8> *excluded_gpus*: render nodes to ignore when auto-detecting GPUs
<9> *gstreamer* audio/video pipeline definitions, see xref:_gstreamer[]
...
Wolf can use every GPU installed in the host, balancing sessions across them while keeping the
rendering (virtual Wayland desktop) and the encoding (GStreamer) on the **same** GPU.

The pool of GPUs is configured with a top-level `[[gpus]]` array; each entry has a `render_node` and a
relative `weight`:
...
When a new session starts Wolf assigns it to the GPU with the highest `(weight - assigned_sessions)`
score, breaking ties with the fewest assigned sessions and finally with the config order. With the
example above, three sessions will land on `renderD128` for every one that lands on `renderD129`.

If the `gpus` array is left empty Wolf will automatically detect **all** the render nodes available on
the host and give each of them a weight of `1`. Use `excluded_gpus` to skip specific devices when
auto-detecting:

[source,toml]
....
# Render nodes to ignore when auto-detecting GPUs (has no effect when `gpus` is set)
...
* *Rendering*: this will use HW acceleration in order to create virtual Wayland desktops and run the chosen app (ex: Firefox, Steam, ...).
...
Since Wolf supports HW accelerated zero-copy pipelines both parts are pinned to the same GPU for each
session.

NOTE: The `WOLF_RENDER_NODE` and `WOLF_ENCODER_NODE` environment variables are still honoured as a
fallback when no GPU can be detected or configured, so single-GPU deployments keep working unchanged.
Per-app `render_node` pins are no longer supported; use the `gpus` list and weights instead.
